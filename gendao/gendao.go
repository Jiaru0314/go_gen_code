...
func Dao(ctx context.Context, in CGenDaoInput, db gdb.DB) (err error) {
	v := g.Cfg().MustGet(ctx, CGenDaoConfig)
	if v.IsSlice() {
		for i := 0; i < len(v.Interfaces()); i++ {
			doGenDaoForArray(ctx, i, in, db)
		}
	} else {
...
func doGenDaoForArray(ctx context.Context, index int, in CGenDaoInput, db gdb.DB) {
	var (
		err error
	)
	if index >= 0 {
		...
		if err != nil {
			log.Fatalf(`invalid configuration of "%s": %+v`, CGenDaoConfig, err)
		}
	}
	if dirRealPath := gfile.RealPath(in.Path); dirRealPath == "" {
		log.Fatalf(`path "%s" does not exist`, in.Path)
	}
	...
	if in.Tables != "" {
		...
		if err != nil {
			log.Fatalf("fetching tables failed: %+v", err)
		}
	}
	// Table excluding.
	if in.TablesEx != "" {
...
func getImportPartContent(ctx context.Context, source string, isDo bool, appendImports []string) string {
	...
	if appendImports != nil && len(appendImports) > 0 {
		...
		if goModPath == "" {
			log.Fatal("go.mod not found in current project")
		}
		mod, err := modfile.Parse(goModPath, gfile.GetBytes(goModPath), nil)
		if err != nil {
			log.Fatalf("parse go.mod failed: %+v", err)
		}
